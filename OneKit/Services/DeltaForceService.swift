...
actor DeltaForceService {
    ...
    func fetchDailyPasswords() async throws -> DailyPwdData {
        ...
        let r = try JSONDecoder().decode(DailyPwdResponse.self, from: data); guard r.ok else { throw DeltaForceError.apiError }; return r.data
    }
...
enum DeltaForceError: LocalizedError {
    case invalidURL; case apiError; case networkError
    var errorDescription: String? {
        switch self { case .invalidURL: return "无效的 URL"; case .apiError: return "API 返回错误"; case .networkError: return "网络请求失败" }
    }
}
