        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 8), count: 8), spacing: 8) {
                ForEach(PaletteColor.presets) { color in
                    RoundedRectangle(cornerRadius: 10).fill(color.color).frame(height: 40)
                        .overlay(RoundedRectangle(cornerRadius: 10).stroke(Color.appSeparator.opacity(0.3), lineWidth: 0.5))
                        .overlay(Image(systemName: "checkmark").font(.caption2).foregroundColor(color.textColor).opacity(viewModel.selectedColor.hex == color.hex ? 1 : 0))
                        .onTapGesture { viewModel.selectPreset(color) }
                }
            }
        }
    }
