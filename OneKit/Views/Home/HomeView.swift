...
struct HomeView: View {
    ...
    private var normalView: some View {
        ...
        .task {
            pwdLoading = true
            if let data = try? await DeltaForceService.shared.fetchDailyPasswords() {
                pwdData = data
            }
            pwdLoading = false
        }
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                Button { showPwd = false } label: {
                    Image(systemName: "xmark").font(.caption2).foregroundColor(.appSecondary)
                        .frame(width: 24, height: 24).background(Color.appCard).clipShape(Circle())
                }
            }
            if let data = pwdData {
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 6) {
                    ForEach(data.codes) { p in
                        ...
                        .background(Color.appCard).clipShape(RoundedRectangle(cornerRadius: 6))
    ...
    private var editView: some View {
        List {
            Section {
                Toggle(isOn: $showPwd) {
                    HStack { Image(systemName: "lock.shield").foregroundColor(.orange); Text("显示三角洲密码") }
                }
            }
    ...
    struct ToolCardView: View {
        ...
        var body: some View {
            VStack(alignment: .leading, spacing: 8) {
                Image(systemName: tool.icon).font(.title2).foregroundColor(Color.iconTint(for: tool.color)).frame(width: 32, height: 32)
                    .background(Color.iconTint(for: tool.color).opacity(0.08)).clipShape(RoundedRectangle(cornerRadius: 8))
